...
import services.ActoresService;
...
public class ComentarioController extends AbstractController {
	...
	public ModelAndView register(@Valid final Comentario credentials, final BindingResult bindingResult, @RequestParam(required = false) final boolean showError) {
		...
		Alumno a = actorService.findByPrincipalAlumn();
		if (a != null) {
			result.addObject("actor", a);
		}else {
			Academia ac = actorService.findByPrincipalAcademy();
	...
	public ModelAndView list_actor() {
		...
		Actores a = actorService.findByPrincipalAlumn();
		if (a != null) {
	...
	public ModelAndView list_actor_suscriptor() {
		...
		for (Integer i : comentarioes) {
			actores.add(actorService.findOne(i));
		}
	...
	public ModelAndView solicitar(@RequestParam final int actorId) {
		ModelAndView result;
		Actores creador = actorService.findOne(actorId);
		Actores suscriptor = actorService.findByPrincipalAlumn();
		if (suscriptor == null) {
			suscriptor = actorService.findByPrincipalAcademy();
		}
	...
	public ModelAndView delete(@RequestParam final int comentarioId) {
		ModelAndView result;
		Comentario comentario = comentarioService.findOne(comentarioId);
		try {
